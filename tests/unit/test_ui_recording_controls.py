...
from taskify.ui.main_window import _TRANSCRIPT_MAX_CHARS, MainWindow
...
class TestToggleRecording:
    ...
    def test_toggle_starts_recording(self, window: MainWindow) -> None:
        ...
        with (
            patch("taskify.ui.main_window.AudioCapture", return_value=mock_ac),
...
class TestSessionTimer:
    def test_tick_formats_time(self, window: MainWindow) -> None:
        ...
        assert (
            window.timer_label._content.replace("configure", "") or True
        )  # Label.configure called; check via dummy
...
class TestLiveTranscript:
    ...
    def test_append_multiple_calls_accumulate(self, window: MainWindow) -> None:
        ...
        assert (
            "First" in window.transcript_box._content
            or "Second" in window.transcript_box._content
        )
